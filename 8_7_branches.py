...
def print_branches(memstruct, root):
    memory = memstruct[0]
    if memory[root][1] != -1:
        print_branches(memstruct, memory[root][1])
    if (memory[root][1] != -1 and memory[root][2] == -1) or (memory[root][1] == -1 and memory[root][2] != -1):
        print(memory[root][0])
    if memory[root][2] != -1:
        print_branches(memstruct, memory[root][2])
