...
if memory[root][2] == 0:
    index = memory[root][1]
    print(memory[index][0])
else:
    current_elem = memory[root]
    prev_elem_key = 0
    while current_elem[2] != -1:
        right_elem_index = current_elem[2]
        prev_elem_key = current_elem[0]
        current_elem = memory[right_elem_index]

    if current_elem[1] == -1:
        print(prev_elem_key)
    else:
        current_elem = memory[current_elem[1]]
        while current_elem[2] != -1:
            right_elem_index = current_elem[2]
            current_elem = memory[right_elem_index]
        print(current_elem[0])
